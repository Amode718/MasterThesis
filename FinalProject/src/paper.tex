...
\begin{compactitem}
  ...
  \item A \emph{traceable data model and APIs} with idempotent re-uploads and stable crop filenames.
...
To situate this project, we survey adjacent tools and how our system compares. In short, we have not found public documentation of a production system that groups handwritten
...
\begin{enumerate}
  \item processes submissions end-to-end,
  \item groups answer images by semantic similarity (no answer OCR\footnote{No answer OCR means that text extracted from the image is not returned in the answer. In this case the answer is a grouping, so text extracted by the AI is not needed.}),
  \item isolates outliers for review,
  \item applies rubrics and propagates score changes consistently, and
  \item supports clipboard-based grade transfer (\emph{Copy Table}) into external systems.
\end{enumerate}

\section{Test Data}
We used a small gold-standard set of 20 pages containing mixed typed and handwritten responses. Known-correct examples provide an interpretation of cluster coherence. The system does not rely  on OCR results to group the answers.
...
Basic flow tests check the process of Upload~$\rightarrow$ process~$\rightarrow$ results, to make sure that it matches the requirements for application flow. 

\emph{Evidence:} Figure~\ref{fig:basic-flow}~(a) shows a set of papers uploaded and processing. (b) shows the grouping results view, and (c) shows the graded group using the ``Rubric Test''. Testing this under several scenarios demonstrates that the process correctly follows the required flow. 

\subsection{Vision-Based Semantic Recognition (No Answer OCR)}

This tests if GPT-4o Vision groups answers \emph{images} into semantically similar groups.

\emph{Evidence:} Figure~\ref{fig:vision-groups} shows three groups created by GPT-4o. In (a) we see identical answers being grouped together as expected. In (b), GPT-4o clustered answers that are semantically the same, with different amounts of work into the same Cluster. (c) shows diverse answers clustered into an ``Ungrouped Answers'' group as expected. 

\subsection{Rubric Propagation \& Clipboard Interoperability}
...
\emph{Evidence:} Figure~\ref{fig:vision-groups}~(c) shows the results of a change in a rubric value assigned to group 1. Near the top, it clearly shows the correct points value of 0.0. 
...
Figure~\ref{fig:copy-table} shows the results of a copy and paste operation that is correctly converted into XML. Note that real names have been redacted. 
...
\begin{tabularx}{\textwidth}{@{}p{11.5cm}p{1.2cm}X@{}}
\toprule
\textbf{Item} & \textbf{OK} & \textbf{Notes} \\
\midrule
Assignment creation (free-form + filled-form options) & $\square$ &  \\
Template binding and identity/answer region editor (filled-form) & $\square$ &  \\
Student upload flow; processing status/notifications & $\square$ &  \\
Grouping view with clusters + \emph{Ungrouped} bucket & $\square$ &  \\
Cluster edit tools (rename, merge, split, reassign) & $\square$ &  \\
Rubric panel; apply/remove rubric items per group & $\square$ &  \\
Totals update immediately after rubric edit & $\square$ &  \\
Copyable grade table (\emph{Copy Table}) & $\square$ &  \\
Pasted table preserves rows/columns in target app & $\square$ &  \\
Audit-friendly identifiers (assignment, question, job id) visible & $\square$ &  \\
Job status is legible during processing (progress, last update, error/hung states) & $\square$ &  \\
\bottomrule
\end{tabularx}
\end{table}
...
\emph{Participants} include 3–6 instructors/TAs familiar with grading. The scope of the questionnaire covers Design/readability only, including layout, labels, hierarchy, contrast, discoverability — \emph{not} speed or task timing.

We use the common likert scale for agreement: 1 - Strongly Disagree, 2 - Disagree, 3 - Neutral, 4 - Agree and 5 - Strongly Agree. 

\paragraph{Likert (1–5) items}
\begin{compactenum}
...
\noindent\emph{Prediction.} Record the model’s shaped output (cluster IDs, memberships, \emph{Ungrouped}).  

\noindent\emph{Teacher-facing evaluation.} We quantify effort as the number of \emph{manual moves} required to correct groupings to the gold (one move = reassigning a single tile). We additionally report ungrouped size and the presence of a dominant ``correct'' cluster when applicable. Formal clustering scores (e.g., B-Cubed, Pairwise F1) are out of scope for this study and omitted to keep the focus on instructor workload.
...
Table~\ref{tab:ui-checklist} was completed for the evaluated build. All required UI elements and flows are present (all boxes checked). These were visually verified by three different individuals. 

\section{Vision-Based Semantic Grouping}

The system groups \emph{answer crops} by visual meaning rather than OCR. Figure~\ref{fig:vision-groups} shows two coherent clusters and an outlier set; each tile is a raw crop. As you can see, the crops represent not only semantically similar answers, but also the formatting of the answers as well. This has the added benefit of allowing a grader to see answers that are identical. 

\subsection{Runtime and Cost}
On a 200-answer batch (20 questions, 10 students), end-to-end grouping/grading completed in approximately 10 minutes ($\approx$20 answers/min; $\approx$3.0~s/answer).
...
\end{tabularx}
\end{center}
Response verbosity drives token variability for GPT-4o-mini; restricting outputs to IDs/memberships yields compact usage comparable to GPT-4o. As we can easily see, the cost to grade a single assignment of 20 questions for 10 students is insignificant to the cost of having a grader work through these papers even if all it saves is 7 minutes ($\$0.79 \div \$7.25/hour = .11$ hours or 6.6 minutes).

\subsection{Workload Reduction: Instructor Actions and Example}

We measure instructor effort as the number of \emph{manual moves} required to correct the model's grouping to a gold clustering (visual meaning). A \emph{move} is a single reassign action of one answer tile. The following vignette is taken from an assignment in Networking \#13-a. We analyze this example in terms of workload reduction across different versions of GPT. 

Table~\ref{tab:clusteringresults} shows a typical question where GPT-4o produces a single coherent correct cluster plus one ungrouped set. However, GPT-4o-mini fragmented the same responses into multiple groups, where the groups differed from multiple tests on the same sample set. The conclusion is clear, the application cannot rely on the cheaper ``mini'' version to reliably group answers. 

\begin{table}[h]
\small
\caption{Clustering using GPT-40 vs. GPT-40-mini.}
...
Table~\ref{tab:instructor-actions} shows the number of moves required for each question on the sample assignment. The results were reliably reproduced in subsequent tests. 

%\paragraph{\emph{Per-question instructor actions (20 Questions $\times$ 10 students).}}
\begin{table}[H]\centering
\caption{Instructor manual moves by question (20 questions $\times$ 10 students; lower is better).}
...
Table~\ref{tab:moves-answer} shows results across 200 answers requiring a total of \emph{49} manual moves, i.e., \emph{24.5 moves per 100 answers}. The per-question median was \emph{2} moves with an interquartile range(IQR) of 1--4. This represents a conservative approach to grouping where the majority of moves are from the ungrouped set to another group. 

\begin{table}[h]
%\begin{center}
\caption{Moves statistics per answer.}
\label{tab:moves-answer}
\small
...
\emph{Participants} included four subjects. One item (Q13) had a missing value; analysis uses available ratings per item.

\paragraph{Central tendency and dispersion:} Across all items, ratings were ceilinged at 5: overall mean \(=4.79\), median \(=5.0\) (Q1 \(=5.0\), Q3 \(=5.0\), IQR \(=0\)). 

\paragraph{Per-item medians and IQR:} Table~\ref{tab:likert-median-iqr} reports median and IQR per item (1--5), treating missing values as \emph{NA}. Most items show perfect consensus (IQR \(=0\)); localized dispersion appears most notably on Q14.

\begin{table}[H]\centering
\caption{Per-item medians and IQR (1–5; \emph{NA} excluded). Item labels abridge the list in \S\ref{chap:evalplan}.}
\label{tab:likert-median-iqr}
\begin{tabularx}{\textwidth}{@{}r c c X@{}}
\toprule
\textbf{Q} & \textbf{Median} & \textbf{IQR} & \textbf{Item (abridged)} \\
\midrule
...
10 & \textbf{4.0} & 0.5 & Edit affordances easy to find \\
...
14 & 4.5 & \textbf{1.5} & Loading/progress indicators clear \\
...
\paragraph{Items needing attention.}
The lowest median was Q10 (\emph{Edit affordances are easy to find}) with \(\tilde{x}=4.0\) (IQR \(=0.5\)). The widest dispersion was Q14 (\emph{Loading/progress indicators are clear}) with IQR \(=1.5\), despite a high median (4.5). These indicate near-ceiling central tendency with pockets of uncertainty around (a) \emph{where/how to rename/merge/split/reassign} and (b) \emph{runtime feedback/telemetry} during processing.

\paragraph{Free-response themes (anonymized).}
\begin{itemize}
  \item \textbf{Scoring polarity help.} Missing or unclear help for negative vs.\ positive scoring semantics.
  \item \textbf{Job status clarity.} Ambiguity in whether jobs are running, complete, or hung when processing.
  \item \textbf{Selection feedback.} Highlighting the selected area in a different color was positively noted.
  \item \textbf{Demo coverage.} Some aspects were “not determinable from the video,” suggesting a documentation/demo gap.
\end{itemize}

\paragraph{Implications.}
Given the ceilinged scores, changes should target \emph{discoverability} and \emph{status legibility}, not broad visual redesign: 
(1) add an inline \emph{grading polarity} tooltip in the rubric panel; 
(2) strengthen signifiers and empty-state nudges for edit affordances (rename/merge/split/reassign); 
(3) improve job-status telemetry (progress \%, “active/hung/complete” badges, timestamped last heartbeat); 
(4) update the demo/video to explicitly show these moments so raters can verify them without inference.

\section{Clipboard Interoperability}
Figure~\ref{fig:copy-table} shows the source grade table and pasted result. 
...
We delivered the integrated web app, background services, a reproducible AI configuration (prompts and model settings), and a \emph{Copy Table} flow that enables fast pasting of grades into external systems (e.g., LMSs or Sheets). Documentation includes an instructor guide and technical deployment notes.
...
\begin{compactitem}
  ...
  \item Future: prompting with answers to identify the correct group.
  \item Future: prompting with rubrics that the AI can assign to individual answers or groups. 
\end{compactitem}
