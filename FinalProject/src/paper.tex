...
Totals update after a rubric edit; \emph{the rubric panel exposes a brief help tooltip clarifying positive vs.\ negative scoring semantics}; the on-screen grade table can be copied to the clipboard and pasted into an LMS/spreadsheet.
...
Job status is legible during processing (progress, last update, error/hung states) & [ ] &  \\
\bottomrule
\end{tabularx}
\end{table}
...
\emph{Participants.} Four raters. One item (Q13) had a missing value; analyses use available ratings per item.

\paragraph{Central tendency and dispersion.}
Across all items, ratings were ceilinged at 5: overall mean \(=4.79\), median \(=5.0\) (Q1 \(=5.0\), Q3 \(=5.0\), IQR \(=0\)). 

\paragraph{Per-item medians and IQR.}
Table~\ref{tab:likert-median-iqr} reports median and IQR per item (1–5), treating missing values as \emph{NA}. Most items show perfect consensus (IQR \(=0\)); localized dispersion appears most notably on Q14.
